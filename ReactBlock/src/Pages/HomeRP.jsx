...
// ПАЛІТРА СИСТЕМИ
const COLORS = {
    blue: '#3b82f6',   // Академія
    purple: '#a855f7', // Баланс
    green: '#10b981',  // Прогрес/Еволюція
    bg: '#050505',
    surface: '#09090b',
    border: 'rgba(255, 255, 255, 0.05)'
};

// --- КОМПОНЕНТ: БІЧНА ПАНЕЛЬ (SIDEBAR) ---
const Sidebar = () => {
    const navItems = [
    ...
    return (
        ...
        >
            ...
            <div className="d-flex flex-column gap-2 flex-grow-1">
                ...
                {navItems.map((item, idx) => (
                    <motion.a 
                        ...
                        style={{ color: '#e4e4e7', transition: 'all 0.2s', border: item.active ? `1px solid ${COLORS.border}` : '1px solid transparent' }}
                    >
            ...
            <div className="mt-auto">
                <div style={{ height: '1px', backgroundColor: COLORS.border, width: '100%', marginBottom: '1rem' }}></div>
                <motion.a href="#" whileHover={{ backgroundColor: 'rgba(255,255,255,0.03)' }} className="d-flex align-items-center p-3 text-decoration-none rounded-3" style={{ color: '#a1a1aa' }}>
                    <i className="fa-solid fa-gear me-3" style={{ width: '20px', textAlign: 'center' }}></i>
                    <span className="fw-semibold">Налаштування</span>
                </motion.a>
                <motion.a href="/accounts/logout/" whileHover={{ backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#ef4444' }} className="d-flex align-items-center p-3 text-decoration-none rounded-3" style={{ color: '#a1a1aa' }}>
                    <i className="fa-solid fa-arrow-right-from-bracket me-3" style={{ width: '20px', textAlign: 'center' }}></i>
                    <span className="fw-semibold">Відключитися</span>
                </motion.a>
            </div>
        </div>
    );
...
// --- КОМПОНЕНТ: КАРТКА ДАШБОРДУ (BENTO BLOCK) ---
const DashboardCard = ({ title, value, subtitle, icon, color, delay }) => (
    <motion.div 
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: delay }}
    ...
    >
        <div style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '40%', background: `linear-gradient(to top, ${color}15, transparent)`, pointerEvents: 'none' }}></div>
        <div className="position-relative z-1 d-flex flex-column h-100">
            ...
            <div className="mt-auto">
                ...
                <p className="font-monospace m-0" style={{ color: color, fontSize: '0.85rem', letterSpacing: '1px' }}>{subtitle}</p>
            </div>
        </div>
    </motion.div>
);
...
const HomeRP = () => {
    ...
    return (
        // ОСЬ ТУТ ДОДАНО ЕФЕКТ ПЛАВНОГО ВХОДУ (Fade-in + Blur)
        <motion.div 
        ...
        >
            ...
            {/* Основна зона (зміщена вправо від сайдбару) */}
            <div className="flex-grow-1" style={{ marginLeft: '280px', padding: '2.5rem 4rem' }}>
                ...
                <div className="d-flex justify-content-between align-items-center mb-5 pb-3" style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                    <div>
                        ...
                        <h1 className="text-white fw-bold m-0" style={{ fontSize: '2rem' }}>Вітаю, <span className="river-text" style={{ background: `linear-gradient(90deg, #fff, ${COLORS.blue}, #fff)` }}>Сінсу</span></h1>
                    </div>
                    <div className="d-flex align-items-center gap-3">
                        <div className="d-flex align-items-center px-3 py-2" style={{ backgroundColor: 'rgba(16, 185, 129, 0.1)', border: `1px solid ${COLORS.green}40`, borderRadius: '20px' }}>
                            <div className="rounded-circle me-2" style={{ width: '8px', height: '8px', backgroundColor: COLORS.green, boxShadow: `0 0 10px ${COLORS.green}` }}></div>
                ...
                {/* --- СІТКА ДАШБОРДУ (BENTO GRID) --- */}
                <div className="row g-4">
                    
                    {/* Великий головний блок (Фокус дня) */}
                    <div className="col-12 col-xl-8">
                        <motion.div 
                            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
                            className="p-5 h-100 position-relative overflow-hidden"
                            style={{ backgroundColor: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: '24px' }}
                        >
                            <div style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: '100%', background: `radial-gradient(circle at right, ${COLORS.blue}10 0%, transparent 80%)`, pointerEvents: 'none' }}></div>
                            ...
                            <div className="position-relative z-1 d-flex flex-column justify-content-center h-100">
                                <div className="d-inline-flex align-items-center px-3 py-1 mb-4" style={{ backgroundColor: `${COLORS.blue}15`, border: `1px solid ${COLORS.blue}30`, borderRadius: '6px', width: 'fit-content' }}>
                                    <i className="fa-solid fa-crosshairs me-2" style={{ color: COLORS.blue, fontSize: '0.85rem' }}></i>
                                ...
                                <div className="d-flex gap-3">
                                    <button className="btn fw-bold px-4 py-3 text-white" style={{ backgroundColor: COLORS.blue, border: 'none', borderRadius: '12px' }}>
                                        Продовжити занурення
                                    </button>
                                    <button className="btn fw-bold px-4 py-3 text-white" style={{ backgroundColor: 'transparent', border: `1px solid ${COLORS.border}`, borderRadius: '12px' }}>
                    ...
                    <div className="col-12 col-md-6 col-xl-4">
                        <DashboardCard title="Індекс Еволюції" value="Lvl. 12" subtitle="450 EXP до наступного рівня" icon="fa-solid fa-dna" color={COLORS.green} delay={0.1} />
                    </div>

                    {/* Статистика Академії */}
                    <div className="col-12 col-md-6 col-xl-4">
                        <DashboardCard title="Компіляція Модулів" value="4 / 28" subtitle="14% від загального ядра" icon="fa-solid fa-cubes" color={COLORS.blue} delay={0.2} />
                    </div>

                    {/* Ритм Балансу */}
                    <div className="col-12 col-md-6 col-xl-4">
                        <DashboardCard title="Стан Потоку" value="Оптимальний" subtitle="Останній відпочинок: 2 год тому" icon="fa-solid fa-fan" color={COLORS.purple} delay={0.3} />
                    </div>

                    {/* Стрік (Активність) */}
                    <div className="col-12 col-md-6 col-xl-4">
                        <DashboardCard title="Синхронізація (Стрік)" value="5 Днів" subtitle="Продовжуйте підтримувати ритм" icon="fa-solid fa-fire" color="#f59e0b" delay={0.4} />
                    </div>
